  ...
  const downloadTransactions = async () => {
    ...
    try {
      ...
      if (downloadFormat === 'pdf') {
        ...
        // ── Page 1: Header ──
        // Wise logo — draw the actual SVG using the path from Logo.jsx
        // Scaled version of the Wise flag/wordmark
        const logoScale = 0.8
        const logoX = 15
        const logoY = 12
        doc.setFillColor(22, 51, 0) // forest green
        // Draw "WISE" wordmark as bold text styled to match the brand
        doc.setFontSize(24)
        doc.setFont('helvetica', 'bold')
        doc.setTextColor(22, 51, 0)
        // Draw the flag mark (simplified)
        const flagW = 12 * logoScale
        const flagH = 12 * logoScale
        doc.setFillColor(22, 51, 0)
        // Triangle shape approximating the Wise flag
        doc.triangle(
          logoX, logoY + flagH * 0.45,
          logoX + flagW * 0.55, logoY,
          logoX + flagW, logoY + flagH,
          'F'
        )
        doc.triangle(
          logoX, logoY + flagH * 0.45,
          logoX + flagW * 0.45, logoY + flagH * 0.75,
          logoX + flagW * 0.55, logoY,
          'F'
        )
        // "wise" text next to the flag
        doc.text('wise', logoX + flagW + 3, logoY + flagH * 0.85)
